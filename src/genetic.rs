...
fn compute_fitness(encoding: &Encoding, test_data: &TestData) -> f64 {
    ...
    for encoding in ng.iter_mut().skip(3) {
        if rand_between(0, 100) < 15 {
            ...
            for sub_encoding in encoding.sub_encodings.iter_mut() {
                ...
                sub_encoding.length_bits -= rand_between(0, sub_encoding.length_bits as usize / 2) as u32;
                sub_encoding.position_bits -= rand_between(0, sub_encoding.position_bits as usize / 2) as u32;
                sub_encoding.expn_id_bits -= rand_between(0, sub_encoding.expn_id_bits as usize / 2) as u32;
